...
class TrajFastDataset(Dataset):
    def __init__(self, city, dates, path, device, is_pretrain):
        ...
        """
        segment_travel_time_dict.npy is used for chengdu_test
        """

        # segment_travel_time_dict = np.load('/data/MaoXiaowei/ODTTE/processed_data/trajectories/cd_2018_segment_travel_time_dict_test.npy', allow_pickle=True).item()
        # shinked_segment_travel_time_dict = dict()
        # for key, value in segment_travel_time_dict.items():
        #     road = key[-1]
        #     shriked_road_id = self.shrink_nonzero_dict[road]
        #     new_key = (key[0], key[1], shriked_road_id)
        #     shinked_segment_travel_time_dict[new_key] = value
        # np.save('/data/HuangYiheng/test/DiffRoute/processed_data/trajectories/cd_2018_shrinked_segment_travel_time_dict_test.npy', shinked_segment_travel_time_dict)
